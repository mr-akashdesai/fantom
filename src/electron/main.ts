...
const logosPath = path.join(
  __dirname,
  '../../src/assets/images/logo/fantom-logo-lg.png'
)
...
app.whenReady().then(() => {
  ipcMain.on('dialog:setThemeSource', async (event, args) =>
    changeThemeSource(args)
  )
  ipcMain.handle('dialog:getSources', getvideoSources)
  ipcMain.handle('dialog:saveRecording', async (event, args) =>
    saveRecording(args)
  )
  ipcMain.on('dialog:openRecording', async (event, args) => openRecording(args))
  ipcMain.handle('dialog:copyToClipboard', async (event, args) =>
