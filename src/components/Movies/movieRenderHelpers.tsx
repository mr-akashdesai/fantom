...
const determineTitle = (sectionType: SectionType) => {
    switch (sectionType) {
        ...
        case SectionType.similar: return 'Similar'
    }
}
        ...
        <div id={type} className={overFlowWrapper ? 'movie__container movie__overflowWrapper' : 'movie__container'}>
            ...
            <div key={index} className="movie__itemContainer" onClick={() => history(`/movie-details/${movie.id}`)}>
                ...
                <div className="movie__itemInfo">
                    ...
                    <span className="movie__releaseDate">{movie.release_date && format(new Date(movie.release_date), 'yyyy')}</span>
                </div>
    ...
    <div className="movies__gridContainer">
        ...
        {data && data.results.map((series: any, index: number) => 
            <div key={index} className="movie__itemContainer" onClick={() => history(`/series-details/${series.id}`)}>
                ...
                <span className="movie__releaseDate">{series.first_air_date && format(new Date(series.first_air_date), 'yyyy')}</span>
                </div>
