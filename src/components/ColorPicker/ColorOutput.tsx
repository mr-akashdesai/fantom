...
const ColorOutput = (color: any) => {
  ...
  const returnConversion = () => {
    ...
    switch (selectedType) {
      case 'hex':
        return convertOutput(convertableColor.toHexString())
      case 'rgba':
        return convertOutput(convertableColor.toRgbString())
      case 'hsla':
        return convertOutput(convertableColor.toHslString())
    }
  }
    ...
    <>
      ...
      <div className='colorPicker__colorOutputContainer'>
        <div className='colorPicker__colorOutputButtonGroup'>
          <Button appearance='subtle' onClick={() => changeType('next')}>
            <MdOutlineArrowDropUp size={'1.5rem'} />
          </Button>
          <Button appearance='subtle' onClick={() => changeType('previous')}>
