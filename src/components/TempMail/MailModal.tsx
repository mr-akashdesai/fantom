...
const MailModal = ({mailjs, showLoginModal, setShowLoginModal, history, setCredentials}: any) =>  {
    ...
    useEffect(() => {
     ...
     const handleLogin = async () => {
        ...
        const login = new Promise((resolve, reject) => {
            ...
            .then((res: any) =>{
            res.data.id && res.data.token
            ? resolve(setCredentials({username: email, password: password, autoGenerated: false}))
            : reject()
          })
        })
     ...
     const handleRegister = async () => {
        ...
        Promise.all([  
            ...
            .then(() => setCredentials({username: emailWithDomain, password: password, autoGenerated: false}))])
        .then(() =>  { 
            setShowLoginModal(false)
            setLoading(false)
        })
     ...
     const handleCreateRandomAccount = async () => {
        ...
        Promise.all([        
            ...
            .then((res: any) => setCredentials({username: res.data.username, password: res.data.password, autoGenerated: true}))])
        .then(() => { 
            setShowLoginModal(false)
            setLoading(false)
        })
    ...
    const EmailAndPassword = () => 
        ...
        {email.length > 1 && emailError && <div className="tempMail__errorMessage">
            <BiErrorCircle/>{emailError}
            </div>}
        <br />
        ...
        <InputGroup inside>
            ...
            <Whisper trigger="focus" speaker={<Tooltip>Required</Tooltip>}>
            <Input placeholder={'password'} type={visibleRepeatPW ? 'text' : 'password'} value={repeatPassword} onChange={(value) => handleRepeatPassword(value)}/>
            </Whisper>
            <InputGroup.Button onClick={() => setVisibleRepeatPW(!visibleRepeatPW)}>
                {visibleRepeatPW ? <EyeIcon /> : <EyeSlashIcon />}
            </InputGroup.Button>
        </InputGroup>
        {password.length > 1 && repeatPassword.length > 1 && passwordError && <div className="tempMail__errorMessage">
            <BiErrorCircle/>{passwordError}
            </div>}
        </>}
        </div>
     ...
     const Login = () => <>
        ...
        {loginError && <div className="tempMail__errorMessage tempMail__loginError"><BiErrorCircle/>Invalid credentials</div>}
        <Button disabled={!email || !password || emailError != null} onClick={handleLogin} appearance="primary" color="green">
        Login
        </Button>
    ...
    const Register = () => <> 
        ...
        <Modal.Footer>
        <div className="tempMail__modalFooterContainer">
        <Button disabled={!email || !password || emailError != null || passwordError !=null} onClick={handleRegister} appearance="primary" color="green">
        Register
        </Button>
        <DividerCreateAccount/>
        </div>
