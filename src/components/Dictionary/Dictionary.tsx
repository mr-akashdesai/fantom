          ...
          {definition && !error && (
            <div className='dictionary__container'>
              <div className='dictionary__word'>
                {definition.word}
                <span className='dictionary__speaker' onClick={() => !!audio && audio.play()}>
                  <GiSpeaker />
                </span>
                ...
                <div key={index} className='dictionary__definitionContainer'>
                  ...
                  <div id={index.toString()} className='dictionary__definitionWrapper'>
                    {meaning.definitions.map((obj: any, index: number) => (
                      <div key={index} className='dictionary__definitionItem'>
                        <div className='dictionary__definitionIndex'>{index + 1}</div>
                        {obj.definition}
                      </div>
                    ))}
                  </div>
                  {meaning.definitions.length > 1 && (
                    <a
                      id={`toggle-${index}`}
                      onClick={() =>
                        toggleOverflowWrapper(index.toString(), `toggle-${index}`, '2rem', 'See more...', 'See less...')
                      }>
                      See more...
                    </a>
                  )}
