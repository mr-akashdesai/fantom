    ...
    <Modal.Body>
      {Object.keys(suggestions).length > 0 && (
        <div className='movie__suggestionContainer'>
          {suggestions
            .filter((suggestion: any) => suggestion.media_type != 'person')
              ...
              switch (suggestion.media_type) {
                case 'movie':
                  return (
                    <div
                      key={index}
                      className='movie__suggestionItem'
                      onClick={() => history(`movie-details/${suggestion.id}`)}>
                      <img
                        className='movie__suggestionItemImage'
                      ...
                      />
                      {suggestion.title}&nbsp;
                      {suggestion.release_date && format(new Date(suggestion.release_date), '(yyyy)')}
                    </div>
                  )
                case 'tv':
                  return (
                    <div
                      key={index}
                      className='movie__suggestionItem'
                      onClick={() => history(`series-details/${suggestion.id}`)}>
                      <img
                        className='movie__suggestionItemImage'
                        src={`${process.env.MOVIE_DB_IMAGE_SMALL_URL}${suggestion.poster_path}`}
                        onError={({ currentTarget }) => {
                      ...
                      />
                      {suggestion.name}&nbsp;
                      {suggestion.first_air_date && format(new Date(suggestion.first_air_date), '(yyyy)')}
                    </div>
                  )
              }
            })}
